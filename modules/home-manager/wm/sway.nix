...
let
  inherit (config.home.sessionVariables) TERMINAL BROWSER BROWSER2;
  modifierBind = "Mod4";
  menu1 = "wofi --show run";
    ...
    wayland.windowManager.sway = {
      ...
      config = rec {
        ...
        window.commands = [
          ...
          {
            command = "floating enable";
            criteria = { app_id = "org.kde.kalarm"; };
          }
          {
            command = "floating enable";
            criteria = { app_id = "org.kde.kronometer"; };
          }
          {
            command = "floating enable";
        ...
        keybindings = lib.mkOptionDefault {
          ...
          "${modifierBind}+Shift+n" = "exec ${BROWSER2}";
          ...
          XF86AudioRaiseVolume =
            "exec pactl set-sink-volume @DEFAULT_SINK@ +5% && pactl get-sink-volume @DEFAULT_SINK@ | head -n 1| awk '{print substr($5, 1, length($5)-1)}' > /tmp/wobpipe";
          ...
          "${modifierBind}+1" =
            "workspace number 1; exec 'echo 1 > /tmp/sovpipe'";
          "${modifierBind}+2" =
            "workspace number 2; exec 'echo 1 > /tmp/sovpipe'";
          "${modifierBind}+3" =
            "workspace number 3; exec 'echo 1 > /tmp/sovpipe'";
          "${modifierBind}+4" =
            "workspace number 4; exec 'echo 1 > /tmp/sovpipe'";
          "${modifierBind}+5" =
            "workspace number 5; exec 'echo 1 > /tmp/sovpipe'";
          "${modifierBind}+6" =
            "workspace number 6; exec 'echo 1 > /tmp/sovpipe'";
          "${modifierBind}+7" =
            "workspace number 7; exec 'echo 1 > /tmp/sovpipe'";
          "${modifierBind}+8" =
            "workspace number 8; exec 'echo 1 > /tmp/sovpipe'";
          "${modifierBind}+9" =
            "workspace number 9; exec 'echo 1 > /tmp/sovpipe'";
          "${modifierBind}+0" =
            "workspace number 10; exec 'echo 1 > /tmp/sovpipe'";
